...
# Copy the root package.json and package-lock.json (if it exists)
COPY package*.json ./
...
# We only need the root package.json for metadata, not for installation
...
# The server code is already configured to use the PORT environment variable
# provided by Cloud Run. We just need to tell Cloud Run what port to check.

# Tell Docker that the container listens on the default Cloud Run port 8080
EXPOSE 8080
